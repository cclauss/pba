...
class CifarModelTrainer(object):
    ...
    def save_model(self, checkpoint_dir, step=None):
        ...
        model_save_name = os.path.join(
            checkpoint_dir, 'model.ckpt') + '-' + str(step)
        save_path = self.saver.save(
            self.session, model_save_name)  # , global_step=step)
        tf.logging.info('Saved child model')
        return model_save_name  # + '-' + str(step)
    ...
    def eval_child_model(self, model, data_loader, mode):
        ...
        while True:
            try:
                # self.session.run([self.m.reset_op, self.meval.reset_op])
    ...
    def _run_training_loop(self, curr_epoch):
        ...
        while True:
            try:
                # self.session.run([self.m.reset_op])
                train_accuracy = helper_utils.run_epoch_training(
                    self.session, self.m, self.data_loader, curr_epoch, metric=False)
                break
            except (tf.errors.AbortedError, tf.errors.UnavailableError) as e:
                tf.logging.info(
                    'Retryable error caught: {}.  Retrying.'.format(e))
    ...
    def _compute_final_accuracies(self, iteration):
        ...
        # valid_accuracy = self.eval_child_model(self.meval, self.data_loader, 'val')
        if self.hparams.eval_test:
            if (iteration >= self.hparams.num_epochs - 1):
                test_accuracy = self.eval_child_model(
                    self.meval, self.data_loader, 'test')
            else:
                test_accuracy = 0
        else:
            test_accuracy = 0
    ...
    def run_model(self, epoch):
        ...
        #         '/cpu:0' if self.hparams.use_cpu else '/gpu:0'):
        valid_accuracy = 0.

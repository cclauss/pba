...
def eval_child_model(session, model, data_loader, mode, metric=True):
    ...
    if len(images) % model.batch_size != 0:
        eval_batches += 1
    if metric:
        tf.logging.info("stream ops: {}".format(
            session.run(model.stream_vars)))
        for i in range(eval_batches):
            eval_images = images[i *
...
def run_epoch_training(session, model, data_loader, curr_epoch, metric=True):
    ...
    if metric:
        ...
        for step in xrange(steps_per_epoch):
            ...
            correct += np.sum(np.equal(np.argmax(train_labels,
                                                 1), np.argmax(preds, 1)))
            count += len(preds)

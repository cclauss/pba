...
class PopulationBasedTraining(FIFOScheduler):
    ...
    def _exploit(self, trial_executor, trial, trial_to_clone):
        ...
        old_config = trial_to_clone.config.copy()
        ...
        # log the exploit
        # from trial_to_clone (epochs) to trial (epochs)
        trial_name = trial_state.orig_tag
        trial_to_clone_name = new_state.orig_tag
        trial_id = ''.join(takewhile(str.isdigit, trial_name))
        trial_to_clone_id = ''.join(takewhile(str.isdigit, trial_to_clone_name))
        trial_path = trial.local_dir + "/policy_" + trial_id + ".txt"
        trial_to_clone_path = trial_to_clone.local_dir + "/policy_" + trial_to_clone_id + ".txt"
        trial_epochs = trial.last_result[TRAINING_ITERATION]
        trial_to_clone_epochs = trial_to_clone.last_result[TRAINING_ITERATION]
        policy = [trial_name, trial_to_clone_name, trial_epochs, trial_to_clone_epochs, old_config["hp_policy"], new_config["hp_policy"]]
        with open(trial.local_dir + "/global.txt", "a+") as f:
            # log new_config
            f.write(str(policy)+"\n")
        # log new_config, overwite trial with clone
        if os.path.exists(trial_to_clone_path):
            copyfile(trial_to_clone_path, trial_path)
        # add line to new trial
